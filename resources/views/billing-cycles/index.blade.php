                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0">
                                            <div class="h-10 w-10 rounded-full {{ $cycle->is_active ? 'bg-green-100' : 'bg-gray-100' }} flex items-center justify-center">
                                                <svg class="h-6 w-6 {{ $cycle->is_active ? 'text-green-600' : 'text-gray-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <div class="flex flex-col items-end">
                                        ...
                                        <div class="mt-2 flex space-x-2">
                                            <a href="{{ route('billing-cycles.show', $cycle) }}" class="text-blue-600 hover:text-blue-900 text-sm font-medium">
                                                View
                                            </a>
                                            <a href="{{ route('billing-cycles.edit', $cycle) }}" class="text-gray-600 hover:text-gray-900 text-sm font-medium">
                                                Edit
                                            </a>
                                            @if(!$cycle->is_active)
                                                <form action="{{ route('billing-cycles.destroy', $cycle) }}" method="POST" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="text-red-600 hover:text-red-900 text-sm font-medium" onclick="return confirm('Are you sure you want to delete this cycle?')">
                                                        Delete
                                                    </button>
                                                </form>
                                            @endif
